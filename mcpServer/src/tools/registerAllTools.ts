...
import { enableMcp, disableMcp, setupProject } from "./setupProject.js";
import { repoBootstrap, type RepoBootstrapConfig } from "./repoBootstrap.js";
import { explainSubdirectories } from "./explainSubdirectories.js";
import { ensureTestPlaybook } from "./testPlaybook.js";
import { listActiveTasks, registerAgent, startTask, completeTask } from "../handlers.js";
import { readFile, getCacheInfo, invalidate, clearCache, toTextContentBlock } from "./contextCache.js";
...
export function registerAllTools(
    ...
    "repo_bootstrap",
    {
      ...
      inputSchema: z.object({
        ...
        config: z.object({
          ...
          architecture: z
            .enum(["monolith", "modular", "hexagonal", "microservices"])
          ...
          overwritePolicy: z
            .enum(["no-overwrite", "overwrite", "prompt"])
            .optional()
            .describe("Conflict policy for existing files"),
        }),
      }),
    },
    ...
    "explain_subdirectories",
    {
      ...
      inputSchema: z.object({
        ...
        language: z
          .string()
          .optional()
          .describe("Output language (e.g. 'es', 'en'). If omitted, inferred from runtime locale."),
        stack: z.string().optional().describe("Optional stack override (python, java, node, etc.)"),
        includeHidden: z
          .boolean()
          .optional()
          .describe("Whether to include hidden directories/files. Default: false"),
        maxDepth: z
          .number()
          .int()
          .min(1)
          .max(32)
          .optional()
          .describe("Maximum recursion depth for subdirectories. Default: 8"),
        overwritePolicy: z
          .enum(["merge", "overwrite", "no-overwrite"])
          .optional()
          .describe("Conflict policy when target file exists. Default: merge"),
        fallbackFilename: z
          .string()
          .optional()
          .describe("Fallback filename when format cannot be inferred. Default: index.md"),
      }),
    },
    async (params) => {
    ...
    "test_playbook",
    {
      ...
      inputSchema: z.object({
        projectPath: z
          .string()
          .optional()
          .describe("Project root or workspace root. Defaults to the current working directory."),
      }),
    },
    async ({ projectPath }) => {
  ...
  server.registerTool(
    ...
    async ({ path: filePath }) => {
      ...
      return { content: [toTextContentBlock(result, meta)] };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ path: filePath }) => {
      ...
      return {
        content: [
          { type: "text" as const, text: removed ? `Evicted: ${filePath}` : `Not in cache: ${filePath}` },
        ],
      };
    },
  );
  ...
  server.registerTool(
    ...
    async () => {
      ...
      return { content: [{ type: "text" as const, text: `Cleared ${count} cached file(s).` }] };
    },
  );
      ...
      if (!flow) {
        return {
          content: [
            {
              type: "text" as const,
              text: JSON.stringify({ error: `Flow "${flowId}" not found or expired` }, null, 2),
            },
          ],
        };
      }
      return { content: [{ type: "text" as const, text: JSON.stringify(flow, null, 2) }] };
    },
  );
  ...
  server.registerTool(
    "sandbox_execute",
    {
      ...
      inputSchema: z.object({
        ...
        language: z.enum(["javascript", "typescript", "python", "bash", "sh"]).describe("Language of the code"),
        ...
        tasks: z
          .array(
            z.object({
              ...
              estimatedCostTokens: z.number().int().min(0).optional().describe("Estimated token cost (default 500)"),
              estimatedTimeMs: z
                .number()
                .int()
                .min(0)
              ...
              critical: z
                ...
                .optional()
                .describe(
                  "If true, this task is on the critical path — sequential treatment when risk is high (default false)",
                ),
              metadata: z.record(z.unknown()).optional().describe("Optional metadata passed through to the agent"),
            }),
          )
          .min(1)
