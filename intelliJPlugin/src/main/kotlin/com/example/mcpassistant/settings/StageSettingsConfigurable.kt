        ...
        val ui = panel {
            ...
            row("MCP Server Path:") {
                textField()
                    ...
                    .comment("Absolute path to the 'mcpServer' directory")
            }
    ...
    private fun restartBridge() {
        com.intellij.openapi.project.ProjectManager.getInstance().openProjects.forEach { project ->
            ApplicationManager.getApplication().executeOnPooledThread {
                val startup = com.example.mcpassistant.plugin.McpInstallStartupActivity()
                val mcpDir = startup.resolveMcpDir(project)
                project.service<com.example.mcpassistant.services.MCPBridgeService>().start(mcpDir)
            }
        }
    }
